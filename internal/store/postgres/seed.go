...
func Seed(storage store.Storage) {
	...
	for _, u := range users {
		if err := storage.Users().Create(ctx, u); err != nil {
			log.Printf("error saving user %s: %v", u.Username, err)
			return
		}
	}
	...
	commnts := generateComments(500, users, posts)
	for _, comment := range commnts {
...
func generateUsers(num int) []*model.User {
	...
	for i := range num {
		users[i] = &model.User{
			...
			Email:    usernames[i%len(usernames)] + fmt.Sprintf("%d@email.com", i),
...
func generatePosts(num int, users []*model.User) []*model.Post {
...
func generateComments(num int, users []*model.User, posts []*model.Post) []*model.Comment {
	cms := make([]*model.Comment, num)
	for i := range num {
		cms[i] = &model.Comment{
			PostID:  posts[rand.Intn(len(posts))].ID,
			UserID:  users[rand.Intn(len(users))].ID,
			Content: seedComments[rand.Intn(len(seedComments))],
		}
	}
	return cms
}
